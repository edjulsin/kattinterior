...
const Guide = (
    ...
    <>
        <section className='max-w-2xs md:max-w-sm xl:max-w-full grid grid-rows-[auto_auto_auto] xl:grid-rows-[1fr_auto] xl:grid-cols-[auto_1fr] gap-x-15 justify-center items-center gap-y-10 xl:gap-y-0'>
            <Image
                className='h-100 md:h-120 max-w-sm w-full row-span-2 object-cover object-center rounded-tl-full rounded-tr-full'
                src={src}
                alt={alt}
                width={width}
                height={height}
        ...
        <section className='flex flex-col gap-y-20 max-w-2xs md:max-w-sm xl:max-w-full'>
            ...
            <ol className='*:max-w-sm grid justify-center items-center auto-rows-fr gap-y-10 md:gap-y-15 xl:gap-y-25 gap-x-30 xl:grid-cols-2 xl:*:row-span-3 xl:*:odd:justify-self-start xl:*:even:justify-self-end'>
                <li key={'first'} className='flex flex-col justify-center gap-y-4 xl:col-start-2 slide-from-bottom anim-delay-[100ms]'>
                    <h3 className='text-xl/loose md:text-2xl/loose font-serif flex flex-col'>
                        <span className='text-2xl md:text-3xl text-gold-900'>00</span>
                        <span>{headTitle}</span>
                    </h3>
                ...
                </li>
                {
                    contents.map(([title, description], i) =>
                        <li key={title} className='flex flex-col justify-center gap-y-4 slide-from-bottom anim-delay-[100ms]'>
                            <h3 className='text-xl/loose md:text-2xl/loose font-serif flex flex-col'>
                                <span className='text-2xl md:text-3xl text-gold-900'>{(i + 1 < 10 ? '0' : '') + (i + 1)}</span>
                                <span>{title}</span>
                            </h3>
